...
REM ------------------------------------------------------------------------
REM Sekcja 4 BACKUP_SHUTDOWN – dodatkowy backup bez nadpisywania + wy³¹czenie komputera
:BACKUP_SHUTDOWN
cls
echo --- BACKUP DANYCH (tryb shutdown) ---
echo.
...
REM Tworzenie znacznika czasu z sekundami (unikalny)
for /f %%i in ('powershell -NoProfile -Command "Get-Date -Format yyyy-MM-dd_HH-mm-ss"') do set "BACKUP_TIMESTAMP=%%i"

REM Docelowy katalog backupu z dat¹ i godzin¹
set "BACKUP_TARGET=%BACKUP_DIR%\backup_shutdown_%BACKUP_TIMESTAMP%"
mkdir "%BACKUP_TARGET%"

REM Plik logu
set "LOGFILE=%LOG_DIR%\backup_shutdown_%BACKUP_TIMESTAMP%.log"

echo Rozpoczynam backup do: "%BACKUP_TARGET%" ...
robocopy "%SOURCE_DIR%" "%BACKUP_TARGET%" /E /DCOPY:T /COPY:DAT /R:3 /W:5 /TEE ^
 /XD "%SOURCE_DIR%\backup" "%SOURCE_DIR%\multimedia" /LOG+:"%LOGFILE%"

echo [%DATE% %TIME%] Backup (shutdown) zakoñczony z kodem %ERRORLEVEL% >> "%LOGFILE%"
if %ERRORLEVEL% GEQ 8 (
    echo [B£¥D] Backup zakoñczony z b³êdem. SprawdŸ log: %LOGFILE%
) else (
    echo Backup zakoñczony sukcesem. Log: %LOGFILE%
...
echo Wy³¹czanie komputera za 60 sekund...
shutdown /s /t 60
pause
goto MENU

REM ------------------------------------------------------------------------
REM Sekcja 5 RESTORE – przywracanie danych
:RESTORE
cls
...
REM ------------------------------------------------------------------------
REM Sekcja 6 ANALIZA – przegl¹d i analiza plików logów
:ANALIZA
cls
echo --- ANALIZA LOGÓW ---
echo.
...
REM Tworzenie raportu z analizy
for /f %%i in ('powershell -NoProfile -Command "Get-Date -Format yyyy-MM-dd_HH-mm-ss"') do set "BACKUP_TIMESTAMP=%%i"
set "REPORT=%LOG_DIR%\analiza_%BACKUP_TIMESTAMP%.txt"

(
    findstr /i /c:"ERROR" "%LOG_DIR%\!ANLFILE!"
    findstr /i /c:"copied" "%LOG_DIR%\!ANLFILE!"
    findstr /i /c:"Extra" "%LOG_DIR%\!ANLFILE!"
    findstr /i /c:"failed" "%LOG_DIR%\!ANLFILE!"
    findstr /i /c:"mismatch" "%LOG_DIR%\!ANLFILE!"
) > "%REPORT%"

type "%REPORT%"
echo.
echo Raport zapisany w: %REPORT%

pause
goto MENU

REM ------------------------------------------------------------------------
REM Sekcja 7 HASH – obliczanie sumy kontrolnej pliku
:HASH
cls
...
REM ------------------------------------------------------------------------
REM Sekcja 8 NOTATKI – uruchamianie trybu zapisu notatek i zasad
:NOTATKI
cls
...
REM ------------------------------------------------------------------------
REM Sekcja 9 KONFIG – zmiana œcie¿ek katalogów
:KONFIG
cls
...
REM ------------------------------------------------------------------------
REM Sekcja 10 FOLDERY – szybkie otwieranie katalogów
:FOLDERY
cls
...
REM ------------------------------------------------------------------------
REM Sekcja 11 KONIEC – zakoñczenie programu
:KONIEC
cls
